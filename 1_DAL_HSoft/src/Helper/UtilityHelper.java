...
public class UtilityHelper {
    ...
    a-z, A-Z, 0-9
     */
    public static boolean checkIdProduct(JTextField txt) {
        ...
        if (id.matches(rgx)) {
            ...
            Helper.DialogHelper.alert(txt.getRootPane(), "Mã danh mục có 1-10 kí tự\nchữ hoa, thường không dấu hoặc số.");
            return false;
        }
    }
    
    public static boolean checkTenDanhMuc(JTextField txt) {
        txt.setBackground(white);
        String id = txt.getText();
        String rgx = ".{3,50}";
        if (id.matches(rgx)) {
            ...
            txt.setBackground(Color.yellow);
            Helper.DialogHelper.alert(txt.getRootPane(), "Tên danh mục phải từ 3-50 kí tự.");
            return false;
        }
    }
